...
- **できること**: 料理プラン / 料理コース / フリードリンク / 会場 / 備品 / 飲物 / 施設情報 / SEO の一覧・追加・編集・削除・並び替え
...
5. 「デプロイを管理 → 編集（鉛筆）→ バージョン: 新バージョン」で更新（**URLは固定のまま**／デプロイは1つでOK）
6. 「今すぐ反映」を使う場合: GitHub の fine-grained PAT（`contents:write` 相当）を発行し、スクリプトプロパティに `GITHUB_PAT` として登録
